...
function Edit_Place({ params }:any) {
    const { t } = useTranslation();
    const [image, setImage] = useState<File | null>(null);
    const [daysOfWork, setDaysOfWork] = useState<string[]>([]);
    const [services, setServices] = useState<{ titleEn: string; titleAr: string; estimatedTime: string }[]>([]);
    ...
    useEffect(() => {
        if (places && params.id) {
            const matchedPlace = places.find((p: any) => p._id === params.id);
    ...
    return (
        <div>
            <Custom_Page_Title title={t('admin.places.edit-place')} />
            <div className='flex justify-end items-center mb-5'>
                <Link href="/admin/places/show" className='btn bg-main text-white'>{t('admin.places.show-places')}</Link>
            </div>
                ...
                <div className="tabs tabs-border">
                    ...
                    <div className="tab-content border-base-300 bg-base-100 p-10">
                        ...
                        <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-100 hover:border-gray-300">
                            ...
                            {place && place.image ? (<img src={`${place.image}`} className='w-18 h-18' />) : (<></>)}
